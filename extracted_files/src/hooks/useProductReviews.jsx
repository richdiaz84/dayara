    ...
    export function useProductReviews(productId, userId) {
      ...
      const fetchReviewsAndPurchaseStatus = useCallback(async () => {
        ...
        try {
          ...
          if (reviewsError) {
            console.error("Supabase reviewsError:", reviewsError);
            throw reviewsError;
          }
          
          const formattedReviews = reviewsData?.map(r => ({
            ...r,
            user_name: r.user?.raw_user_meta_data?.full_name || r.user?.email || 'Anónimo'
          })) || [];
          ...
          if (userId) {
            ...
            if (ordersError) {
              console.error("Supabase ordersError:", ordersError);
              throw ordersError;
            }
